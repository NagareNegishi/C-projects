...
// test evaluate_hand
TEST_CASE(0, 0, HIGH_CARD)
TEST_CASE(1, 0, ONE_PAIR)
TEST_CASE(2, 0, TWO_PAIR)
TEST_CASE(3, 0, THREE_OF_A_KIND)
TEST_CASE(4, 0, FULL_HOUSE)
TEST_CASE(5, 0, FOUR_OF_A_KIND)
TEST_CASE(6, 0, FIVE_OF_A_KIND)
void test_evaluate_hand(const int index, const int joker, Type expected){
    Card test_data[][5] = {
    ...
    Type result = evaluate_hand(&test_data[index], joker);
    TEST_ASSERT_EQUAL(expected, result);
}
...
TEST_CASE(12, 14 , 1) // FIVE_OF_A_KIND vs FIVE_OF_A_KIND with joker
void test_compare_hand(const int index1, const int index2, int expected)
{
    Hand test_hands[] = {
        ...
        {{SIX, SIX, SIX, SIX, JOKER}, 550, 1, FIVE_OF_A_KIND, true} // five of kind but jokers is weakest
    };
